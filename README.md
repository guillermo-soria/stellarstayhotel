...
GET /api/rooms/available and POST /api/reservations.
...
Base rate per room type per day:

Junior $60, King $90, Presidential $150.

Weekend uplift: +25% on Saturdays/Sundays (per-night basis).

Length discounts: applied per day after weekend uplift:
...
Breakfast option: +$5 per guest per day.
...
Bootstrap minimal project (folders, Express, Pino, Zod).

Implement PricingEngine and date utilities.

Implement use cases (GetAvailableRooms, CreateReservation) and corresponding ports.

Create adapters: Prisma repositories, HTTP controllers, and logger integration.

Add reliability patterns: timeouts, retries (where applicable), idempotency.

Write unit tests (pricing engine) and minimal E2E happy-path tests for both endpoints.

Provide a clear README with setup steps and Docker instructions.

Later: generate OpenAPI spec from Zod schemas using zod-to-openapi.
